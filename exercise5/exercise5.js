...
Table.prototype = {
	createNameField: function(count) {
		...
		nameField.setAttribute('id', 'row_' + count);
		return nameField;
	},
	...
	createEmailField: function(count) {
		...
		emailField.setAttribute('id', 'row_' + count);
		return emailField;
	},

	createEmailLabel: function(row_id, content) {
		var emailLabel = document.createElement('label');
		emailLabel.setAttribute('class', 'email');
		emailLabel.setAttribute('id', 'row_' + row_id);
	...
	createNameLabel: function(row_id, content) {
		...
		nameLabel.setAttribute('id', 'row_' + row_id);
	...
	performSave: function(rowID, saveButton){
		...
		var nameField = currentRow.querySelector('td input.name');
		var emailField = currentRow.querySelector('td input.email');  
	...
	createSaveButton: function(count) {
		...
		saveField.setAttribute('id', 'row_' + count );
	...
	performEdit: function(rowID) {
		...
		var nameValue = currentRow.querySelector('td label.name').innerHTML;
		var emailValue = currentRow.querySelector('td label.email').innerHTML;
	...
	createEditLink: function(count){
		...
		editLink.setAttribute('id', 'row_' + count );
	...
	createDeleteLink: function(count) {
		...
		deleteLink.setAttribute('id', 'row_' + count );
		deleteLink.appendChild(document.createTextNode('Delete'));

		deleteLink.addEventListener('click', function() {
			currentRow = document.querySelector('tr#'+ this.id);
			currentRow.remove();
		});
